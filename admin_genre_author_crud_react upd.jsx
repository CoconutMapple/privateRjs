      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-100 via-sky-50 to-white p-6">
        ...
        <div className="max-w-4xl mx-auto bg-white/90 backdrop-blur-md shadow-2xl rounded-3xl p-8 border border-blue-100">
            ...
            <div>
              ...
              <div className="grid gap-3">
                {genres.map((genre) => (
                  <div
                    key={genre.id}
                    className="p-4 bg-gradient-to-r from-blue-50 to-sky-50 rounded-2xl border border-blue-100 shadow-sm flex justify-between items-center gap-3"
                  >
                    <span>{genre.name}</span>
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          const newName = prompt("Update genre:", genre.name);
                          if (newName) updateGenre(genre.id, newName);
                        }}
                        className="px-3 py-1 bg-yellow-400 rounded-lg text-white"
                      >
                        Edit
                      </button>
                      <button
                ...
                {authors.map((author) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          const newName = prompt("Update author:", author.name);
                          if (newName) updateAuthor(author.id, newName);
                        }}
                        className="px-3 py-1 bg-yellow-400 rounded-lg text-white"
                      >
                        Edit
          ...
          <div className="hidden">
            ...
            <p>Test Tab Switch</p>
            <p>Test Update Function</p>
            <p>Test Delete Function</p>
          </div>
        </div>
      </div>
    </>
